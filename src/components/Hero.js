...
const fitting = require("../assets/fitting.png");

const Hero = () => {

    const icons = [riego, agrotextil, herramientas, jardineria, piscina, fitting];
                ...
                {["Riego agrícola", "Agrotextil", "Herramientas", "Jardinería", "Piscina", "Fitting"].map(
                    (text, index) => (
                        <button
                            ...
                            className="px-4 py-2 flex items-center gap-2 font-semibold text-sm text-neutral-600 bg-neutral-300 hover:bg-white transition-colors duration-300 rounded-full"
                        >
                            <img src={icons[index]} alt={text} width={20} height={20} />
                            {text}
                        </button>
                    )
                )}
