    ...
    return (
        <section id="hero" ref={heroRef} className="relative w-full z-0 h-screen flex items-center justify-center">
            ...
            <div className="relative h-full z-10 flex flex-col md:flex-row items-center text-white w-4/5">
                {/* Columna izquierda */}
                <div className="w-7/12 h-full flex flex-col justify-center items-stretch text-center md:text-left gap-8">
            ...
            <div className="absolute bottom-20 flex gap-4 font-Inter z-40">
                {["Riego", "Agrotextiles", "Tecnología", "Soporte", "Consultoría", "Innovación"].map(
                    (text, index) => (
                        <button
                            key={index}
                            ref={(el) => (btnsRef.current[index] = el)}
                            className="px-4 py-2 text-neutral-600 hover:text-black bg-neutral-400 hover:bg-white transition-colors duration-300 rounded-full"
                        >
                            {text}
                        </button>
                    )
